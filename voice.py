...
async def ban(interaction: discord.Interaction, member: discord.Member):
    ...
    if interaction.user.voice is None:
        ...
        if member.voice is None or member.voice.channel != channel:
            ...
            await interaction.response.send_message(f"{member.display_name} has been banned from {channel.name}!", ephemeral=True, delete_after=20)
...
async def unban(interaction: discord.Interaction, member: discord.Member):
    if interaction.user.voice is None:
        ...
        await interaction.response.send_message('This user is not banned!', ephemeral=True, delete_after=20)
    else:
        channel = interaction.user.voice.channel
        if member.voice is None or member.voice.channel != channel:
            await channel.set_permissions(member, connect=True)
            await interaction.response.send_message(f"{member.display_name} has been unbanned from {channel.name}!", ephemeral=True, delete_after=20)
        elif member.voice.channel == channel:
            await channel.set_permissions(member, connect=True)
            await interaction.response.send_message(f"{member.display_name} has been unbanned from {channel.name}!", ephemeral=True, delete_after=20)
